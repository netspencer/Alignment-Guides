...
;(function(a) {
  ...
  function i(a) {
    ...
    for (j = 0; j < c[b].length; j++) {
      ...
      if (g.scope == e || g.scope == 'all') {
        k = g.mods.length > 0
        for (i in d)
          if ((!d[i] && h(g.mods, +i) > -1) || (d[i] && h(g.mods, +i) == -1))
            k = !1
        ;((g.mods.length == 0 && !d[16] && !d[18] && !d[17] && !d[91]) || k) &&
          g.method(a, g) === !1 &&
  ...
  function l(a, b, d) {
    ...
    for (i = 0; i < e.length; i++) {
      ...
      ;(a = a[0]),
        (a = g[a] || a.toUpperCase().charCodeAt(0)),
        a in c || (c[a] = []),
        c[a].push({ shortcut: e[i], scope: b, method: d, key: e[i], mods: h })
    }
  }
  function m(a) {
